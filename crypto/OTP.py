import sys, re, binascii
#why the the one-time pad should be thrown away -- Dagan Harrington '13

#problem : each of the ciphertexts & target have been encrypted
#        :using the same one-time pad
#        :given only the ciphertexts, find the plaintext of target

#usage : each level of the output will contain a small array of [a-zA-z0-9].
#guess and move on to the next character spot
#some statistical sampling would help to nudge out the guesswork step.

ciphertext1 = ''
ciphertext2 = '234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f'
ciphertext3 = '32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb'
ciphertext4 = ''
ciphertext5 = '3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070'
ciphertext6 = ''
ciphertext7 = ''
ciphertext8 = '315c4eeaa8b5f8bffd11155ea506b56041c6a00c8a08854dd21a4bbde54ce56801d943ba708b8a3574f40c00fff9e00fa1439fd0654327a3bfc860b92f89ee04132ecb9298f5fd2d5e4b45e40ecc3b9d59e9417df7c95bba410e9aa2ca24c5474da2f276baa3ac325918b2daada43d6712150441c2e04f6565517f317da9d3'
ciphertext9 = ''
ciphertext10= '466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83'
target      = '32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904'

ls = [ciphertext1, ciphertext2, ciphertext3, ciphertext4,
      ciphertext5, ciphertext6, ciphertext7, ciphertext8,
      ciphertext9, ciphertext10]

#algorithm:
#for each ciphertext[1-10]:
#for each two hex bits in ciphertext[i] and target
#xor them together and store value in a list by their position
#manually parse through output to find likely candidates

#weaknesses:
#-not possible if the 'plaintext' is not ascii or reasonably grammatical english
#-a more sophisticated method (ie crib-dragging) should, principally, be better

def test5():
    targetptext = {int : []} #dictionary to associate location with char
    for i in range(170):#why need to initialize like this?
        targetptext[i] = []
        
    for cipher in ls:
        cipher = cipher[0:len(target)]
        for pos in xrange(0,len(target),2):
            c = cipher[pos:pos+2]
            t = target[pos:pos+2]
            ch = hex(int(c,16)^int(t,16))[2:]
            if len(ch) == 1:
                ch = '0'+ch
            print c,'xor',t,'=',ch.decode('hex')
            targetptext[pos].append(ch.decode('hex'))
    for pos in xrange(0,len(targetptext),2):
        #print pos, targetptext[pos]
        print [elem for elem in targetptext[pos] if elem.isalpha() or elem.isdigit()]
        
test5()

#the following are other tries, incomplete tries and different avenues

"""
def test2(): ##crib-drag testing for spaces
    targetptext = {} #dictionary to associate location with char

    a = ' '.encode('hex')
    #the text to match against (cipherB xor cipherB)
    #if ((cipherB xor cipherB)@position i) is an uppercase letter
    #and you xor it with ' ', you get the lowercase identical letter
    #converse also holds
    
    for i in range(500):#initialize all indices to ' '
        targetptext[i] = '_'
    

    tcxor = hex(int(ciphertext1, 16) ^ int(target, 16))[2:]
    
    for pos in range(len(str(tcxor))-5):
        section1 = str(tcxor)[pos:pos+len(a)]
        print '#',pos, '  ', section1.decode('hex'), ' xor ', a.decode('hex'), ' = ',
        x = str(hex(int(section1, 16) ^ int(a, 16)))[2:]
        if len(x) % 2 != 0:
            x = x+'0' #pad to avoid decode('hex') receiving odd length string ##must be better way
        print x.decode('hex')
        if section1.decode('hex').lower() == x.decode('hex').lower():
            print 'TRUE!!!!!!',
            print section1.decode('hex').lower()
            targetptext[pos] = section1.decode('hex').lower()
            print targetptext[pos]
    for i in range(500):
        print targetptext[i],

def test3(): ##crib-drag testing for longer words
    targetptext = {} #dictionary to associate location with char

    a = ' the '.encode('hex')
    #begin by dragging ' the ' and see what in m1 xor m2 looks convincing
    #process should be iterative
    

    tcxor = hex(int(ciphertext1, 16) ^ int(ciphertext3, 16))[2:-1]
    print len(str(tcxor))
    
    for pos in range(len(str(tcxor))):
        section1 = str(tcxor)[pos:pos+len(a)]
        print '#',pos, '  ', section1, ' xor ', a, ' = ',
        x = str(hex(int(section1, 16) ^ int(a, 16)))[2:]
        if len(x) % 2 != 0:
            x = x+'0' #pad to avoid decode('hex') receiving odd length string ##must be better way
        print x.decode('hex')
"""
